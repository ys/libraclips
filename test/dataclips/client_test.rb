...
class Dataclips::ClientTest < Minitest::Unit::TestCase
  ...
  def test_fetch_is_not_nil_with_correct_params
    VCR.use_cassette('dataclips') do
      [
        ...
        refute_equal nil, Dataclips::Client.new.fetch(url_or_id)
      end
    end
  end
  ...
  def test_fetch_has_correct_values
    VCR.use_cassette('dataclips') do
      result = Dataclips::Client.new.fetch('jcopmmuubebhyotlbspulagvghxx')
